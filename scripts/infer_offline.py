...
def load_settings(args):
    ...
    if args.input is None:
        if args.input_type != "cropped" or not data_config.get("training"):
            raise ValueError("--input is required unless cropped mode has data.training in config")
    ...
    args.output_video = (
        ...
        if args.output_video else output_root / f"{args.input_type}_detections.mp4"
    )
    args.timeline_png = (
    ...
    with mcap_path.open("rb") as stream:
        ...
        for _, channel, message in reader.iter_messages(topics=[args.topic]):
            ...
            if index % args.frame_stride == 0:
                ...
                yield f"{args.input}#timestamp={message.log_time}", frame
            index += 1
        if not found_topic:
...
def iter_rosbag(args):
    ...
    while reader.has_next():
        ...
        if index % args.frame_stride == 0:
            ...
            yield f"{args.input}#timestamp={timestamp}", frame
        index += 1
...
def paste_patch(canvas, patch, mask):
    ...
    resized = cv2.resize(patch, (x2 - x1 + 1, y2 - y1 + 1), interpolation=cv2.INTER_AREA)
...
def main():
    ...
    try:
        if args.input_type == "cropped":
            progress = tqdm(
                iter_cropped_groups(args), total=args.max_frames,
            ...
            processed_frames = 0
            progress = tqdm(
                factories[args.input_type](args), total=args.max_frames,
        ...
        final_frames = int(round(args.timeline_seconds * args.output_fps))
        for _ in range(final_frames):
            video.write(final_timeline)
    finally:
        video.close()
