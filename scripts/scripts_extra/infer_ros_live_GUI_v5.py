...
class LiveRosAnomalyInfer(Node):
    def __init__(self, args):
        ...
        t0 = time.time()
        self.models, self.thresholds = load_models_and_thresholds(self.areas, args, self.device, log_fn=self.vlog)
        ...
        if args.publish_rulex:
            self.rulex_pub = self.create_publisher(RulexDetectionResult, args.rulex_topic, 10)
            self.vlog(1, f"[publisher] publishing RulexDetectionResult to {args.rulex_topic}")

        self.anomaly_map_pub = None
        if args.publish_anomaly_map:
            self.anomaly_map_pub = self.create_publisher(CompressedImage, args.anomaly_map_topic, 10)
            self.vlog(1, f"[publisher] publishing anomaly map to {args.anomaly_map_topic}")

        self.dashboard_pub = None
        if args.publish_dashboard:
            self.dashboard_pub = self.create_publisher(CompressedImage, args.dashboard_topic, 10)
            self.vlog(1, f"[publisher] publishing dashboard to {args.dashboard_topic}")

        self.timeline_pub = None
        if args.publish_timeline:
            self.timeline_pub = self.create_publisher(CompressedImage, args.timeline_topic, 10)
            self.vlog(1, f"[publisher] publishing timeline to {args.timeline_topic}")
        ...
        self.first_anomaly_publish_done = False
        self.first_dashboard_publish_done = False
        self.first_timeline_publish_done = False
        ...
        self.score_history = OrderedDict((area, deque(maxlen=self.timeline_history_len)) for area in self.areas)
        self.latest_results = OrderedDict((area, {"status": "waiting"}) for area in self.areas)
        ...
        if self.args.show_model_input:
            cv2.namedWindow("ADVIS Dashboard", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("ADVIS Dashboard", 1200, 800)

        if self.show_timeline:
            cv2.namedWindow("ADVIS Timeline", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("ADVIS Timeline", 1000, 500)
    ...
    def publish_rulex_result(self, results, frame_bgr, corr_frame_id, corr_stamp):
        ...
        for area_name in self.areas:
            ...
            if hasattr(area_msg, "frame_id"):
                area_msg.frame_id = corr_frame_id
            if hasattr(area_msg, "stamp"):
        ...
        if self.args.attach_image_on_anomaly and any_anomaly and hasattr(msg, "image"):
            msg.image = self.bridge.cv2_to_imgmsg(frame_bgr, encoding="bgr8")
            msg.image.header = frame_bgr.header if hasattr(frame_bgr, "header") else msg.image.header
    ...
    def publish_image_topic(self, pub, img_bgr, header, kind="image"):
        if pub is None or img_bgr is None:
            return
        msg = self.bridge.cv2_to_imgmsg(img_bgr, encoding="bgr8")
        msg.header = header
        pub.publish(msg)

        if kind == "anomaly" and not self.first_anomaly_publish_done:
            print(f"[publish] FIRST anomaly map published on {self.args.anomaly_map_topic}")
            self.first_anomaly_publish_done = True
        elif kind == "dashboard" and not self.first_dashboard_publish_done:
            print(f"[publish] FIRST dashboard published on {self.args.dashboard_topic}")
            self.first_dashboard_publish_done = True
        elif kind == "timeline" and not self.first_timeline_publish_done:
            print(f"[publish] FIRST timeline published on {self.args.timeline_topic}")
            self.first_timeline_publish_done = True
    ...
    def process_latest_frame(self):
        ...
        try:
            ...
            for area in self.areas:
                r = results[area]
                if "score" in r:
                    msg_parts.append(f"{area}: score={r['score']:.5f}, norm={r['norm_score']:.3f}, status={r['status']}")
                else:
            ...
            if self.args.publish_dashboard or self.args.publish_anomaly_map or self.args.show_model_input:
                dashboard, recon_full, anom_full = draw_dashboard_panel(
                    frame_bgr,
                    area_inputs,
                    self.latest_results,
                    frame_id=msg_id,
            ...
            if self.args.publish_anomaly_map and anom_full is not None:
                self.publish_image_topic(self.anomaly_map_pub, anom_full, msg.header, kind="anomaly")

            if self.args.publish_dashboard and dashboard is not None:
                self.publish_image_topic(self.dashboard_pub, dashboard, msg.header, kind="dashboard")

            if self.args.show_model_input and dashboard is not None:
            ...
            if self.show_timeline or self.args.publish_timeline:
                ...
                if self.args.publish_timeline:
                    self.publish_image_topic(self.timeline_pub, panel, msg.header, kind="timeline")
                if self.show_timeline:
